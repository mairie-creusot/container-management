...
const { setThreecxConfig, clearThreecxConfig } = await import("../src/services/setupStore.js");
...
describe("Routes 3CX — config (aucun secret ne ressort)", () => {
  it("GET /api/3cx/config : baseUrl + clientId, JAMAIS le clientSecret", async () => {
    ...
    expect(response.json()).toEqual({ configured: true, config: { baseUrl: "https://pbx.exemple.fr:5001", clientId: "quai-xapi" } });
  ...
  it("PUT /api/3cx/config : connexion valide = persistée, réponse redactée", async () => {
    ...
    expect(response.json()).toEqual({
      configured: true,
      config: { baseUrl: "https://pbx.exemple.fr:5001", clientId: "quai-xapi", tlsRejectUnauthorized: false },
    });
    expect(response.body).not.toContain(CLIENT_SECRET);
  });
