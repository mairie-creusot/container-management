...
import { publicManifest } from "@quai/plugin-contract";
import type { Plugin, PublicPluginManifest } from "@quai/plugin-contract";
...
export default async function pluginsRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get<{ Params: { id: string } }>("/api/plugins/:id/config", async (request, reply) => {
    const plugin = getPlugin(request.params.id);
